...
def _make_installconfig(script, osobj, unattended_data, arch, hostname, url):
    ...
    if script.requires_user_password() and not unattended_data.get_user_password():
        raise RuntimeError(_("%s requires the user-password to be set.") % osobj.name)
    config.set_user_password(unattended_data.get_user_password() or "")

    # Set the admin-password:
    # In case it's required and not passed, just raise a RuntimeError.
    if script.requires_admin_password() and not unattended_data.get_admin_password():
        raise RuntimeError(_("%s requires the admin-password to be set.") % osobj.name)
...
class OSInstallScript:
    ...
    def have_new_libosinfo():
        ...
        for script in win7.get_install_script_list():
            if Libosinfo.InstallScriptInjectionMethod.CDROM & script.get_injection_methods():
                return True
    ...
    def __init__(self, script, osobj, osinfomediaobj, osinfotreeobj):
        ...
        if not OSInstallScript.have_new_libosinfo():  # pragma: no cover
            raise RuntimeError(
                _("libosinfo or osinfo-db is too old to " "support unattended installs.")
            )
    ...
    def set_preferred_injection_method(self, namestr):
        ...
        if injection_method & supported_injection_methods == 0:
            raise RuntimeError(
                _("OS '%(osname)s' does not support required " "injection method '%(methodname)s'")
    ...
    def requires_user_password(self):
        return self._requires_param(Libosinfo.INSTALL_CONFIG_PROP_USER_PASSWORD)

    def requires_admin_password(self):
        return self._requires_param(Libosinfo.INSTALL_CONFIG_PROP_ADMIN_PASSWORD)
    ...
    def generate(self):
        if self._osinfomediaobj:
            return self._script.generate_for_media(self._osinfomediaobj, self._config)
        if hasattr(self._script, "generate_for_tree") and self._osinfotreeobj:
            # osinfo_install_script_generate_for_tree() is part of
            # libosinfo 1.6.0
            return self._script.generate_for_tree(self._osinfotreeobj, self._config)

        return self._script.generate(self._osobj.get_handle(), self._config)

    def generate_cmdline(self):
        if self._osinfomediaobj:
            return self._script.generate_command_line_for_media(self._osinfomediaobj, self._config)
        if hasattr(self._script, "generate_command_line_for_tree") and self._osinfotreeobj:
            # osinfo_install_script_generate_command_line_for_tree() is part of
            # libosinfo 1.6.0
            return self._script.generate_command_line_for_tree(self._osinfotreeobj, self._config)
        return self._script.generate_command_line(self._osobj.get_handle(), self._config)
    ...
    def write(self):
        fileobj = tempfile.NamedTemporaryFile(prefix="virtinst-unattended-script", delete=False)
...
def _lookup_rawscripts(osinfo, profile, os_media):
    ...
    if os_media:
        if not os_media.supports_installer_script():
            ...
            raise RuntimeError(
                _("OS '%s' media does not support unattended " "installation") % (osinfo.name)
            )
    ...
    if not script_list:
        raise RuntimeError(_("OS '%s' does not support unattended installation.") % osinfo.name)
    ...
    if profile:
        ...
        if not rawscripts:
            raise RuntimeError(
                _(
                    "OS '%(osname)s' does not support unattended "
    ...
    log.debug("Install scripts found for profile '%s': %s", profile, ", ".join(ids))
    return rawscripts


def prepare_install_scripts(guest, unattended_data, url, os_media, os_tree, injection_method):
    ...
    scripts = []
    rawscripts = _lookup_rawscripts(guest.osinfo, unattended_data.profile, os_media)

    osinfomediaobj = os_media.get_osinfo_media() if os_media else None
    osinfotreeobj = os_tree.get_osinfo_tree() if os_tree else None

    for rawscript in rawscripts:
        script = OSInstallScript(rawscript, guest.osinfo, osinfomediaobj, osinfotreeobj)
