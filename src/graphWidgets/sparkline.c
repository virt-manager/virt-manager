		      ...
		      gpointer extra)
{
  ...
  priv = GTK_SPARKLINE_GET_PRIVATE (cell);
  ...
  */
  ...
  pixels_per_point = (double)widget->allocation.width / ((double)data->n_values-1);

  points = g_new(GdkPoint, data->n_values);
  for (index=0;index<data->n_values;index++) {
    double cx = ((double)index * pixels_per_point);
    double cy = get_y (&widget->allocation, data, index);
  ...
  for (index = 1 ; index < NTICKS ; index++) {
    gdk_draw_line(widget->window,
		  ...
		  1,
		  widget->allocation.height/NTICKS*index,
