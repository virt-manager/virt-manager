...
class vmmAddHardware(vmmGObjectUI):
    ...
    def _set_disk_controller(self, disk):
        ...
        if disk.bus == "nvme":
            ...
            if nvme_controllers:
                disk.vmm_controller = nvme_controllers[0]
            else:
    ...
    def _build_storage(self):
        ...
        # NVMe disks require a serial number that matches the controller serial
        if bus == "nvme":
            # NVMe disks need explicit controller binding via serial number
            nvme_controllers = [c for c in self.vm.xmlobj.devices.controller if c.type == "nvme"]
            if nvme_controllers:
                # Use the serial from the first NVMe controller
                disk.serial = nvme_controllers[0].serial
            else:
                # Controller will be auto-created with serial="0"
                disk.serial = "0"
